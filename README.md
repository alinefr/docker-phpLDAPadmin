...
[hub]: https://hub.docker.com/r/alinefr/phpldapadmin/

Latest release: 1.2.6.7 - phpLDAPadmin 1.2.6.7 - [Changelog](CHANGELOG.md) | [Docker Hub](https://hub.docker.com/r/alinefr/phpldapadmin/) 

**A docker image to run phpLDAPadmin.**
> [phpldapadmin.sourceforge.net](http://phpldapadmin.sourceforge.net)

- [alinefr/phpldapadmin](#alinefrphpldapadmin)
  ...
  - [Advanced User Guide](#advanced-user-guide)
    - [Extend alinefr/phpldapadmin:1.2.6.7 image](#extend-alinefrphpldapadmin1267-image)
    - [Make your own phpLDAPadmin image](#make-your-own-phpldapadmin-image)
    - [Tests](#tests)
    - [Kubernetes](#kubernetes)
    - [Under the hood: alinefr/web-baseimage](#under-the-hood-alinefrweb-baseimage)
  - [Security](#security)
  - [Changelog](#changelog)
...
Run a phpLDAPadmin docker image by replacing `ldap.example.com` with your ldap host or IP :
    ...
    docker run -p 6443:443 \
            ...
            --detach alinefr/phpldapadmin:1.2.6.7
...
Example script:
    ...
    docker run --name phpldapadmin-service --hostname phpldapadmin-service --link ldap-service:ldap-host --env PHPLDAPADMIN_LDAP_HOSTS=ldap-host --detach alinefr/phpldapadmin:1.2.6.7
...
By default HTTPS is enable, a certificate is created with the container hostname (it can be set by docker run --hostname option eg: phpldapadmin.my-company.com).
	...
	docker run --hostname phpldapadmin.my-company.com --detach alinefr/phpldapadmin:1.2.6.7
...
Add --env PHPLDAPADMIN_HTTPS=false to the run command :
    ...
    docker run --env PHPLDAPADMIN_HTTPS=false --detach alinefr/phpldapadmin:1.2.6.7
...
To fix that run the container with `--copy-service` argument :
		...
		docker run [your options] alinefr/phpldapadmin:1.2.6.7 --copy-service
...
Example command to run the container in `debug` mode:
	...
	docker run --detach alinefr/phpldapadmin:1.2.6.7 --loglevel debug

See all command line options:

	docker run alinefr/phpldapadmin:1.2.6.7 --help
...
- **PHPLDAPADMIN_LDAP_HOSTS**: Set phpLDAPadmin server config. Defaults to :
  ...
  If you want to set this variable at docker run command add the tag `#PYTHON2BASH:` and convert the yaml in python:
		...
		docker run --env PHPLDAPADMIN_LDAP_HOSTS="#PYTHON2BASH:[{'ldap.example.org': [{'server': [{'tls': True}]},{'login': [{'bind_id': 'cn=admin,dc=example,dc=org'}]}]}, 'ldap2.example.org', 'ldap3.example.org']" --detach alinefr/phpldapadmin:1.2.6.7
...
### Extend alinefr/phpldapadmin:1.2.6.7 image

If you need to add your custom TLS certificate, bootstrap config or environment files the easiest way is to extends this image.

Dockerfile example:

    FROM alinefr/phpldapadmin:1.2.6.7
...
Clone this project :
	...
	git clone https://github.com/alinefr/docker-phpLDAPadmin
...
### Under the hood: alinefr/web-baseimage

This image is based on alinefr/web-baseimage.
More info: https://github.com/alinefr/docker-web-baseimage
